...
from NeuralNetwork2 import Neural_Network
...
if __name__ == '__main__':
    ...
    initial_hidden_layer_weights = np.array([[-0.28, -0.22,-0.10], [0.08, 0.20,0.10], [-0.30, 0.32,0.10], [0.10, 0.01,0.10]])
    initial_output_layer_weights = np.array([[-0.29, 0.03, 0.21], [0.08, 0.13, -0.36],[0.12,-0.25,0.04]])
    initial_hidden_node_bias = np.array([-0.02,-0.20,0.10])
    initial_output_node_bias = np.array([-0.33,0.26,0.06])

    nn = Neural_Network(n_in, n_hidden, n_out, initial_hidden_layer_weights, initial_output_layer_weights,
                        learning_rate,initial_hidden_node_bias,initial_output_node_bias)
    ...
    delta_output_layer_weights, delta_hidden_layer_weights,delta_hidden_node_bias,delta_output_node_bias = nn.backward_propagate_error(instances[0],hidden_layer_outputs,output_layer_outputs,onehot_encoded[0])
    nn.update_weights(delta_output_layer_weights,delta_hidden_layer_weights,delta_hidden_node_bias,delta_output_node_bias)
